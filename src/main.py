...
def main(args):
    ...
    if args.preprocess:
        ...
        complex_train_files = dp.write_tensors(
            train, params["processed"], "complex_train"
        )
        complex_test_files = dp.write_tensors(test, params["processed"], "complex_test")

        # Write the complex files to json
        write_json(
            complex_train_files, os.path.join(params["processed"], "complex_train.json")
        )
        write_json(
            complex_test_files, os.path.join(params["processed"], "complex_test.json")
        )
    if args.running:
